...
const sortedByBirthYear = inventors.sort((secondInventor, firstInventor) => secondInventor.year - firstInventor.year)
...
console.log('Exercise 5 My Result: ', people)
...
const isAdultPresent = devs.some(developer => {
    return developer.year >= 18
})
...
const isEveryone19OrOlder = devs.every(developer => {
    return developer.year === 19
})
...
const commentById = comments.find(specificID => {
    return specificID.id === 823423
})
...
const idx = comments.findIndex(indexOfId => {
    return indexOfId.id === 123523
})
